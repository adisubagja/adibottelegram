...
if Config.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == borg.uid and query.startswith("@userbot"):
            rev_text = query[::-1]
...
All instaructions to run @userbot in your PC has been explained in https://github.com/SpEcHiDe/userbot""",
                buttons=[
                    [custom.Button.url("Join the Channel", "https://telegram.dog/userbot"), custom.Button.url(
